    ...
    var init = function (json_data) {
        ...
        for (var i = 0 ; i < mydata.length; i++) {
            for (var j = 0; j < orgs.length; j++) {
                ...
                if (found) {
                    alldata.push({ key1: mydata[i].key, key2: found.key, value: found.value.commits ,value2:found.value.commits/mydata[i].value.totalCommits});
                } else {
                    alldata.push({ key1: mydata[i].key, key2: orgs[j].key, value: 0,value2:0 });
                }
            }
        }
        console.log(alldata);
