...
"#9ACD32"]
        ...
        camera.addEventListener('gamepadbuttondown', function (e) {
            if (e.detail.index === 0) { //x 
                //fires a click event.
                var intel = e.target.children[0].components["raycaster"]
                if (intel && intel.intersectedEls && intel.intersectedEls.length > 0) {
                    intel.intersectedEls[0].click();
                }

            }
            else if (e.detail.index === 3) {//tr
                var intel = e.target.children[0];
                //toggle geometry visibility 
                var bool = intel.getAttribute("visible");
                bool = (bool === "true" || bool === "True" || bool === true) ? true : false;
                intel.setAttribute("visible", !bool);
